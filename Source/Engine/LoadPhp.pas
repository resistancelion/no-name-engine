...
uses
  Windows, SysUtils, hzend_types,
  WideStrUtils, Variants, vcl.dialogs;

const
  DllPHP = 'php7ts.dll';
  ZEND_MODULE_API_NO = 20160303;
...
function OffsetStackAsm(a: Pointer; i: Integer): Pointer;
...
begin
  ...
  value^.value.lval := 1;
  value^.u1.v._type := value^.u1.type_info;
end;
...
begin
  ...
  ShowMessage('ZValArrayKeyFind' + idx.ToString());
...
LoadPHPFunc(@ZvalGetArgs, 'zend_get_parameters_ex');
...
{ LoadPHPFunc(@zend_wrong_paramers_count_error,
  'zend_wrong_paramers_count_error@@12');
