...
uses System.Rtti,System.SysUtils, System.TypInfo, vcl.dialogs, Winapi.Windows, hzend_types;
...
begin
  Self._PropertyType := __PropType;
  Self.TypeData := __PropType.Handle.TypeData;
...
type
  TT = record
    ...
    CharCode: Word;
    Unused: Word;
	  CharCodeUnusedFiller: POINTER;
    KeyData: Longint;
    KeyDataFiller: POINTER;
    Result: INT_PTR;
  end;
var
  P: PByte;
...
begin
  ...
  begin
    ...
    if ParamInfos^[I]^.Kind = tkClass then
      ...
      TValue.Make(Pointer(PNativeUInt(Data)^), ParamInfos[I]^, MethodValues[Zerk])
    else
      TValue.Make(Data, ParamInfos[I]^, MethodValues[Zerk]);
  ...
  begin
    ...
    if System.TypInfo.TParamFlags(P[0]) * [pfVar, pfOut] <> [] then
      //TValue.Make(Pointer(PNativeUInt(Data)^), ParamInfos[I]^, MethodValues[Zerk])
     // ShowMessage( MethodValues[i].ToString);
     // PPointer(PNativeUInt(Data)^)^ := PPointer(MethodValues[i].GetReferenceToRawData^)^;
    begin
      // ~ = AV
      TValue.Make(Pointer(PNativeUInt(Data)^), ParamInfos[I]^, tmp);
      if(tmp.ToString <> MethodValues[I].ToString) then

    // PPointer(PNativeUInt(Data)^)^ := PWord( ORD( 'd'));
    //  new(PT);
     // Right := ord('A');

   //   PPointer(PNativeUInt(Data)^)^ := PAnsiChar('d');


    //  Right := PPointer(PNativeUInt(Data)^)^;

        GetCodePointer()
     // PPointer(( PNativeUInt(NativeInt( PNativeUInt (Data)) + 10))^)^ := PAnsiChar('E');


      //PPointer( Right^ - 4)^  := PAnsiChar('E');

  // PPointer(PNativeUInt(Data)^ - 2)^ := PAnsiChar('E');

      // PPointer(PAnsiChar(PPointer(Data)^[1]))^ := PChar('h');


//ShowMessage( PAnsiChar(PNativeUInt(Data)^ ) ); // d

     // PPointer(@(PWord(@Data)^) ^ := nil;

    //  PT :=pPointer(PNativeUInt(@Data)^)^;
      //ShowMessage(PT.CharCode.ToString);

      //PT.CharCode := ord('g');
     // PT.Msg :=  $0100;
    //  PT.Result
//    ShowMessage( CHR( PT.CharCode) );
    //ShowMessage( PAnsiChar(PNativeUInt(Data)^)  );
   //   PPAnsiChar(PNativeUInt(Data)^)^ := PAnsiChar('C');

     // ShowMessage( PAnsiChar(PNativeUInt(Data)^) );
         // PPAnsiChar(PNativeUInt(Data)^)^ := 'eg';
          //PAnsiChar(PAnsiChar(@Data)^)^ := AnsiChar('f');
          //  PPointer(PNativeUInt(Data)^)^ := PPointer(MethodValues[i].GetReferenceToRawData^)^;

         // PPointer(PNativeUInt(Data)^)^ := PChar('f');

     //   PPointer(PNativeUInt(Data)^)^ := PPointer(MethodValues[i].GetReferenceToRawData^)^;


    //    PPointer(PNativeUInt(Data)^)^ := PPChar('d')^;
