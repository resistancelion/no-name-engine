...
ZEND_API zend_class_entry *lookup_class_ce(zend_class_entry *ce, const char *property_name)
{
	zend_class_entry *original_ce = ce;
	zend_property_info *info;
	unsigned int property_length = strlen(property_name);


	while (ce) {
		if ((info = (zend_property_info*)zend_hash_str_find_ptr(&ce->properties_info, property_name, property_length)) != NULL && (info->flags & ZEND_ACC_SHADOW) != ZEND_ACC_SHADOW) {
			return ce;
		}
		ce = ce->parent;
	}
...
ZEND_API int isset_property(zval *object, const char *property_name)
{
	unsigned int property_length;
	property_length = strlen(property_name);
...
ZEND_API int class_exists(const zval *class_name)
{
	zend_class_entry *ce;
...
ZEND_API zval* read_property22(zval *elem, char *name, int flags) {
	...
	zval property;
	ZVAL_STRINGL(&property, name, strlen(name));
	
	res = Z_OBJ_HT_P(elem)->read_property(elem, &property, flags ? BP_VAR_IS : BP_VAR_R, NULL, &tmp);
...
static zend_always_inline size_t safe_address2(size_t nmemb, size_t size, size_t offset)
{
	int overflow;
...
ZEND_API void* safe_emalloc2(size_t nmemb, size_t size, size_t offset)
{
	return emalloc_rel(safe_address2(nmemb, size, offset));
}

ZEND_API void pZVAL_NEW_REF(zval *p, zval *p2) {
	ZVAL_COPY(p, p2);
}
ZEND_API zval *NewPzval(zval *z) {
	zval *value;
...
ZEND_API zval * ZvalEmalloc(size_t nmemb){
	return safe_emalloc(nmemb, sizeof(zval), 0);
}


ZEND_API void __call_function(zval *func, zval *argv, int argc)
{
	zval retval;
...
ZEND_API void PHPInitSetValue(const char *name,  const char *new_value, int modify_type, int stage) {
	zend_string *ini_name, *ini_value;
	
	
	ini_name = zend_string_init(name, sizeof(name) - 1, 0);
	ini_value = zend_string_init(new_value, sizeof(new_value) - 1, 0);
	
	zend_alter_ini_entry(ini_name, ini_value, modify_type, stage);
	zend_string_release(ini_name);
	zend_string_release(ini_value);
}

ZEND_API zend_class_entry *__create_php_object(zend_string *classname, zval *return_value TSRMLS_DC, zval *__params, int argc) {
...
ZEND_API int update_property_zval(zval *object, const char *property_name, zval *value)
{
	unsigned int property_length;
	property_length = strlen(property_name);
	zend_class_entry *ce, *old_scope;
