...
#define StrNameSpace_Name(class_name, class_length, ToWrite, found) \
	unsigned int i, j;                                              \
	j = 0;                                                           \
	i = class_length;                                                 \
	found = false;                                                     \
	ToWrite = (char *)(class_name + class_length - 1);                  \
	while (i > 0) {                                                      \
		if ((*ToWrite) == '\\') {                                         \
			if (j > 0) {                                                   \
				found = true;                                               \
				int ToWrite_length = class_length - j - 1;                   \
				ToWrite = (char *)emalloc(ToWrite_length + 1);                \
				memcpy(ToWrite, class_name, ToWrite_length);                   \
				ToWrite[ToWrite_length] = 0;                                    \
			}                                                                    \
			break;                                                                \
		}                                                                          \
		ToWrite--;                                                                  \
...
#define StrClassName(class_name, class_length, ToWrite, found) \
    unsigned int i, j;                                          \
    i = class_length;                                            \
    found = false;                                                \
    ToWrite = (char *)(class_name + class_length - 1);             \
    while (i > 0) {                                                 \
        if ((*ToWrite) == '\\') {                                    \
            found = true;                                             \
            int ToWrite_length = class_length - i;                     \
            ToWrite = (char *)emalloc(ToWrite_length + 1);              \
            memcpy(ToWrite, class_name + i, ToWrite_length);             \
            ToWrite[ToWrite_length] = 0;                                  \
            break;                                                         \
        }                                                                   \
        ToWrite--;                                                           \
