...
ZEND_API zval* cdecl zend_hash_index_find2(const HashTable *ht, zend_ulong h)
{
	return zend_hash_index_find(ht, h);
}


ZEND_API zval* cdecl zend_hash_index_findZval(zval *ht, zend_ulong h)
{
	return zend_hash_index_find(Z_ARRVAL_P(ht), h);
}

ZEND_API zval * cdecl zend_symtable_findTest(zval *ht, zend_string *key)
{
	return zend_symtable_find(Z_ARRVAL_P(ht), key);
}

ZEND_API const char* cdecl ZvalGetPChar(zval *z) {
	return (const char*)Z_STRVAL_P(z);
}

ZEND_API void cdecl ZvalSetPChar(zval *z, const char * p, int l, int b) {
	ZVAL_NEW_STR(z, zend_string_init(p, l, b));
	
	//z->value.str = zend_string_init(p, strlen(p), 0);
//	z->u1.type_info = IS_STRING_EX;
}
/*
ZEND_API void rZVAL_UNDEF(zval *z){
	ZVAL_UNDEF(z);
}

ZEND_API void rZVAL_NULL(zval *z){
	ZVAL_NULL(z);
}

ZEND_API void rZVAL_FALSE(zval *z){
	ZVAL_FALSE(z);
}

ZEND_API void rZVAL_TRUE(zval *z){
	ZVAL_TRUE(z);
}

ZEND_API void rZVAL_BOOL(zval *z, int a){
	ZVAL_BOOL(z, a);
}

ZEND_API void rZVAL_LONG(zval *zm int a){
	ZVAL_LONG(z, a);
}

ZEND_API void rZVAL_DOUBLE(zval *z, double d){
	ZVAL_DOUBLE(z, d);
}

ZEND_API void rZVAL_STR(zval *z, zend_string *str){
	ZVAL_STR(z, str);
}


ZEND_API void rZVAL_NEW_STR(zval *z, zend_string *s){
	ZVAL_NEW_STR(z, s);
}


ZEND_API void rZVAL_STR_COPY(zval *z zend_string *s){
	ZVAL_STR_COPY(z, s);
}


ZEND_API void rZVAL_ARR(zval *z, HashTable *obj_ht){
	ZVAL_ARR(z, obj_ht);
}


ZEND_API void rZVAL_NEW_ARR(zval *z){
	ZVAL_NEW_ARR(z);
}


ZEND_API void rVAL_NEW_PERSISTENT_ARR(zval *z){
	ZVAL_NEW_PERSISTENT_ARR(z);
}


ZEND_API void rZVAL_OBJ(zval *z, zend_object *o){
	ZVAL_OBJ(z, o);
}


ZEND_API void rZVAL_RES(zval *z, zend_resource *r){
	ZVAL_RES(z, r);
}

ZEND_API void rZVAL_REF(zval *z, zend_reference *r){
	ZVAL_REF(z, r);
}

ZEND_API void rZVAL_NEW_EMPTY_REF(zval *z){
	ZVAL_NEW_EMPTY_REF(z);
}

ZEND_API void rZVAL_NEW_REF(zval *z, zval *r){
	ZVAL_NEW_REF(z, r);
}
*/
ZEND_API zend_bool cdecl zend_hash_index_existsZval(zval *ht, zend_ulong h)
{
	return zend_hash_index_exists(Z_ARRVAL_P(ht), h);
}
