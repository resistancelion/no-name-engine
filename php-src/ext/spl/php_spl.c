...
PHP_METHOD(RTTIFunc, __construct);
PHP_METHOD(RTTIFunc, __set);
PHP_METHOD(RTTIFunc, __get);
PHP_METHOD(RTTIFunc, DOwnerSelf);
PHP_METHOD(RTTIFunc, DOwnerClass);
PHP_METHOD(RTTIFunc, __call);
PHP_METHOD(RTTIFunc, __callStatic);


static zend_class_entry *ClassdSQLLoadMyRTTI;


ZEND_BEGIN_ARG_INFO_EX(node_php_set_args, 0, 0, 2)
ZEND_ARG_INFO(0, member)
ZEND_ARG_INFO(0, value)
ZEND_END_ARG_INFO()

ZEND_BEGIN_ARG_INFO_EX(node_php_get_args, 0, 1, 1)
ZEND_ARG_INFO(0, member)
ZEND_END_ARG_INFO()

ZEND_BEGIN_ARG_INFO_EX(node_dSQLLoadMyRTTI, 0, 0, 1)
ZEND_ARG_INFO(0, ClassName)
ZEND_END_ARG_INFO()

ZEND_BEGIN_ARG_INFO_EX(node_php_call_args, 0, 0, 2)
	ZEND_ARG_INFO(0, fname)
	ZEND_ARG_INFO(0, arguments)
ZEND_END_ARG_INFO()

ZEND_BEGIN_ARG_INFO_EX(node___callstatic, 0, 0, 1)
	ZEND_ARG_INFO(0, name)
	ZEND_ARG_INFO(0, arguments)
ZEND_END_ARG_INFO()

PHP_METHOD(ClassdSQLLoadMyRTTI, dSQLLoadMyRTTI);
static const zend_function_entry ClassdSQLLoadMyRTTI_methods[] = {
	PHP_ME(ClassdSQLLoadMyRTTI, dSQLLoadMyRTTI, node_dSQLLoadMyRTTI, ZEND_ACC_PUBLIC | ZEND_ACC_STATIC)
	PHP_FE_END
};


static const zend_function_entry DClassMyRTTI_methods[] = {
	PHP_ME(RTTIFunc, __construct, NULL, ZEND_ACC_PUBLIC | ZEND_ACC_FINAL)
	PHP_ME(RTTIFunc, __set, node_php_set_args, ZEND_ACC_PUBLIC | ZEND_ACC_FINAL)
	PHP_ME(RTTIFunc, __get, node_php_get_args, ZEND_ACC_PUBLIC | ZEND_ACC_FINAL)
	PHP_ME(RTTIFunc, __call, node_php_call_args, ZEND_ACC_PUBLIC | ZEND_ACC_FINAL) 
	PHP_ME(RTTIFunc, DOwnerSelf, NULL, ZEND_ACC_STATIC | ZEND_ACC_PUBLIC)
	PHP_ME(RTTIFunc, DOwnerClass, NULL, ZEND_ACC_STATIC | ZEND_ACC_PUBLIC)
	PHP_ME(RTTIFunc, __callStatic, node___callstatic, ZEND_ACC_STATIC | ZEND_ACC_PUBLIC)
	PHP_FE_END
};



int read_property(zval *result, zval *object, const char *property_name, int flags)
{
	...
	ce = Z_OBJCE_P(object);
	if (ce->parent) {
		ce = lookup_class_ce(ce, property_name);
	}
	...
	zval property;
	ZVAL_STRINGL(&property, property_name, strlen(property_name));
...
int fetch_property(zval *result, zval *object, const char *property_name)
{
	if (isset_property(object, property_name)) {
		read_property(result, object, property_name, 0);
		return 1;
	}
...
#define SelfObject(Self1, Self2) \
	zval __tmp1, __tmp2; \
	ReadProp("__DObjectSelf", &__tmp1); \
	ReadProp("__DClassSelf", &__tmp2); \
	Self1 = __tmp1.value.lval; \
	Self2 = __tmp2.value.lval; \
	zval_dtor(&__tmp1); \
	zval_dtor(&__tmp2);


#define ReadProp(name, _zval) \
	fetch_property(_zval, getThis(), name); \

#define updateProp(name, _zval) \
	update_property_zval(getThis(), name, _zval);
...
const char *active_class_name() {
	zend_function *func;

	if (!zend_is_executing()) return "";
...
ZEND_API void CreateCll(zval *result, const char *class_name, int self) {
	zend_string *fclass_name;
	fclass_name = zend_string_init(class_name, strlen(class_name), 0);
	
	zend_class_entry *ce;
	ce = zend_lookup_class(fclass_name);

	int a = 0;
	if (ce != NULL) {
		a = (ce->ce_flags & (ZEND_ACC_INTERFACE | (ZEND_ACC_TRAIT - ZEND_ACC_EXPLICIT_ABSTRACT_CLASS))) == 0;
	}
	
	if(a == 0) {
		zend_class_entry ce2;
		static zend_class_entry *tmp;
		INIT_CLASS_ENTRY(ce2, "", DClassMyRTTI_methods);
		ce2.name = zend_new_interned_string(zend_string_init(class_name, strlen(class_name), 1));
		tmp = zend_register_internal_class(&ce2);
		zend_declare_property_long(tmp, "__DObjectSelf", strlen("__DObjectSelf"), 0, ZEND_ACC_PUBLIC TSRMLS_CC);
		zend_declare_property_long(tmp, "__DClassSelf", strlen("__DClassSelf"), 0, ZEND_ACC_PUBLIC TSRMLS_CC);
	}
	
	zval *args;
	args = safe_emalloc(1, sizeof(zval), 0);
	ZVAL_LONG(&args[0], self);
	
	__create_php_object(fclass_name, result TSRMLS_CC, args, 1);
} 
...
PHP_METHOD(RTTIFunc, __call) {
	int self1, self2;
	SelfObject(self1, self2);

	char *function;
	size_t function_len;
	zval *args;
	PARSE_PARAMS(__call, "sa", &function, &function_len, &args);
	...
	if(_TestCreate->_TestVariant(&Result, self1, self2, function, args)) {
		RETURN_ZVAL(&Result, 1, 0);
	} else {
		zend_error(E_ERROR, "Failed Call function of object [%s] of class %s", function, active_class_name());
	}
	zval_ptr_dtor(&Result);
}
...
PHP_METHOD(ClassdSQLLoadMyRTTI, dSQLLoadMyRTTI) {
	...
	PARSE_PARAMS(dSQLLoadMyRTTI, "s", &class_name, &class_length);
	
	if (strstr(&class_name[0], "DClass\\") != NULL) {
		zend_class_entry ce;
		static zend_class_entry *tmp;
		INIT_CLASS_ENTRY(ce, "", DClassMyRTTI_methods);
		ce.name = zend_new_interned_string(zend_string_init(class_name, class_length, 1));
		tmp = zend_register_internal_class(&ce);
		zend_declare_property_long(tmp, "__DObjectSelf", strlen("__DObjectSelf"), 0, ZEND_ACC_PUBLIC TSRMLS_CC); 
		zend_declare_property_long(tmp, "__DClassSelf", strlen("__DClassSelf"), 0, ZEND_ACC_PUBLIC TSRMLS_CC);
	}
}
...
PHP_METHOD(RTTIFunc, __construct) {
	...
	PARSE_PARAMS(__construct, "z", &m);

	int self = _TestCreate->_FindControlVCL(active_class_name());
	
	if (self != 0) {
		if(ZEND_NUM_ARGS() != 0) {
			zval  settl;
			settl.u1.type_info = IS_LONG;
			settl.value.lval = self;
			updateProp("__DObjectSelf", &settl);

			if(Z_TYPE_P(m) == IS_LONG) {
				updateProp("__DClassSelf", m);
			} else {
				zval Result;
				ZVAL_NULL(&Result);	
				
				if (_TestCreate->_TestVariant(&Result, self, -1, "Create", (Z_TYPE_P(m) ? m : NULL))) {
					convert_to_long(&Result);
					updateProp("__DClassSelf", &Result);
				} else {
					zend_error(E_ERROR, "Error [Create] of class %s", active_class_name());
				}
				zval_ptr_dtor(&Result);
			}
		}
	} else {
		zend_error(E_ERROR, "Error [Find] of class %s", active_class_name());
	}
}
...
PHP_METHOD(RTTIFunc, __callStatic) {
	zval * args, Result;
	char * method_name;
	int method_name_len, self;

	self = _TestCreate->_FindControlVCL(active_class_name());
	
	if (self != 0) {
		PARSE_PARAMS(__callStatic, "sz", &method_name, &method_name_len, &args);
		ZVAL_NULL(&Result);	
		if (_TestCreate->_TestVariant(&Result, self, self, method_name, args)) {	
			RETURN_ZVAL(&Result, 1, 0);
		} else {
			zend_error(E_ERROR, "Failed Call static function [%s] of object of class %s", method_name, active_class_name());
		}
	} else {
		zend_error(E_ERROR, "Error [Find] of class %s", active_class_name());
	}
...
PHP_METHOD(RTTIFunc, DOwnerClass) {
	const char * classname = active_class_name();
	
	int self = _TestCreate->_FindControlVCL(classname);
	if(self != 0) {
		CreateCll(return_value, classname, self);
	} else {
		zend_error(E_ERROR, "Error [Find] of class %s", classname);
	}
}        
...
PHP_METHOD(RTTIFunc, __set) {
	...
	size_t Name_len;
	zval *Value;
	PARSE_PARAMS(__call, "sz", &Name, &Name_len, &Value);
	
	
	if ((self1 != 0) && (self2 != 0)) {
		zval Result;
		ZVAL_NULL(&Result);	
		if (_TestCreate->_TestVariant(&Result, self1, self2, Name, Value)) {	
			RETURN_ZVAL(&Result, 1, 0);
		} else {
			zend_error(E_USER_WARNING , "Property %s of class %s cannot be write", Name, active_class_name());
		}
		zval_ptr_dtor(&Result);
	}else {
		zend_error(E_USER_WARNING , "Property %s of class %s cannot be write", Name, active_class_name());
...
PHP_METHOD(RTTIFunc, __get) {
	...
	PARSE_PARAMS(__get, "s", &ret, &ret_length);
	if ((self1 != 0) && (self2 != 0)) {
		zval Result;
		ZVAL_NULL(&Result);	
	

		if(_TestCreate->_TestVariant(&Result, self1, self2, ret, NULL)) {
			RETURN_ZVAL(&Result, 1, 0);
		} else {
			zend_error(E_USER_WARNING , "Property %s of class %s cannot be read", ret, active_class_name());
		}
...
PHP_MINIT_FUNCTION(spl)
{
	...
	zend_class_entry ce;
	INIT_CLASS_ENTRY(ce, "ClassdSQLLoadMyRTTI", ClassdSQLLoadMyRTTI_methods);

	ClassdSQLLoadMyRTTI = zend_register_internal_class(&ce);
